...
contract PubSub{
    ...
    mapping(uint => mapping(uint => PubData [])) public pubData;

    // subscription mapping
    mapping(uint => mapping(uint => uint [])) public subData;
    ...
    // fn to publish data
    function publishData(uint _pubid, uint _topicid, uint _eventid, string memory hash) public{
    ...
    function retHash(uint _pubid, uint _topicid, uint _eventid) public view returns(string memory) {
